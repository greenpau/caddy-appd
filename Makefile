...
build:
	@mkdir -p bin/
	@rm -rf ./bin/$(APP_NAME)
	@go build -v -o ./bin/$(APP_NAME) cmd/$(APP_NAME)/main.go;
	@./bin/$(APP_NAME) version
	@./bin/$(APP_NAME) list-modules --versions | grep $(APP_NAME)
...
qtest: covdir
	@echo "$@: started"
	@time richgo test -v -coverprofile=.coverage/coverage.out -run Test* ./pkg/services/...
	@go tool cover -html=.coverage/coverage.out -o .coverage/coverage.html
	@go tool cover -func=.coverage/coverage.out | grep -v "100.0"
	@echo "$@: complete"
